...
import esprit.pi.demo.entities.Baha.Forum;
import esprit.pi.demo.Services.Baha.IForumService;
...
public class ForumController {
    ...
    public ResponseEntity<?> UpdateForum(@RequestBody Forum Forum , @PathVariable Long id_forum) {
        if (iforumservice.existByIdForum(id_forum)) {
            esprit.pi.demo.entities.Baha.Forum forum = iforumservice.getForumById(id_forum).orElseThrow(
                    ()->new EntityNotFoundException(("Requested Forum not found"))
            );
